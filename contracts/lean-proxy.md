...
| DefaultExecutor generation retries | C10 deleted same-URL temporal retries and fixed sleeps for 429/502/503/504. Those raw responses retain status, body, and `Retry-After` for the request-scoped account planner. Distinct configured fallback URLs and pooled transports remain; one refreshed-credential attempt on 401/403 is explicitly transitional until C13 unifies its budget. |
