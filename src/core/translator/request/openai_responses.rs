...
pub fn openai_responses_to_chat_request(
    ...
    // Custom tools declared as {type:"custom"} or custom_tool_call input
    // items — retained for the response conversion (JS 116, 196-198, 232).
    ...
    for item in &input_items {
        ...
        match item_type {
            ...
            Some("function_call") | Some("custom_tool_call") => {
                ...
                if is_custom {
                    if let Some(n) = item.get("name").and_then(|v| v.as_str()) {
                        if !n.trim().is_empty() {
                            custom_tool_names.insert(n.trim().to_string());
                        }
                    }
                }
                    ...
                    match item.get("input") {
                        Some(Value::String(s)) => {
                            ...
                            if trimmed.starts_with('{') {
                                ...
                                serde_json::to_string(&serde_json::json!({"input": s}))
                                    .unwrap_or_else(|_| "{}".to_string())
                            }
                        }
                        Some(v) => serde_json::to_string(&serde_json::json!({"input": v}))
                            .unwrap_or_else(|_| "{}".to_string()),
                        None => coerce_arguments(item.get("arguments")),
                    }
                } else {
                    coerce_arguments(item.get("arguments"))
                };
            ...
            Some("function_call_output") | Some("custom_tool_call_output") => {
                ...
                pending_reasoning.clear();
                pending_reasoning_encrypted.clear();
                let output = coerce_output(item.get("output"));
                result["messages"]
                    .as_array_mut()
                    .unwrap()
                    .push(serde_json::json!({
    ...
    // Convert tools format. Hosted tools (e.g. {type:"request_user_input"})
    // carry no name and can't be chat functions — filter them out (JS 177-180).
    // Custom tools ({type:"custom"}) become functions with one raw `input`
    // string; names are retained in `_customToolNames` for the response
    // conversion (JS 190-198, 232).
    ...
    if let Some(tools) = body.get("tools").and_then(|v| v.as_array()) {
        let converted: Vec<Value> = tools.iter().filter_map(|tool| {
            ...
            if tool.get("type").and_then(|v| v.as_str()) == Some("custom") {
                ...
                let format_hint = [
                    tool.get("format").and_then(|f| f.get("syntax")).and_then(Value::as_str),
                    tool.get("format").and_then(|f| f.get("definition")).and_then(Value::as_str),
                ]
                .into_iter()
                .flatten()
                .filter(|s| !s.is_empty())
    ...
    let mut all_custom: Vec<String> = custom_tool_names
        .into_iter()
        .chain(seen_custom.into_iter())
    ...
    if !all_custom.is_empty() {
        result["_customToolNames"] =
            Value::Array(all_custom.into_iter().map(Value::String).collect());
    }
