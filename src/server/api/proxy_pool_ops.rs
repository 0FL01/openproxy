//! Proxy pool operations: relay test + Vercel deploy routes.
//!
//! Restored verbatim from `src/server/api/mitm_config.rs` as of `548b9b38^`.
//! Commit `548b9b38` ("remove interception infrastructure") deleted that whole
//! module including its `.merge()`, which silently deregistered these pool
//! routes as collateral. The MITM control/cert handlers are intentionally NOT
//! restored, and neither are the cloudflare/deno deploy handlers (no test or
//! dashboard coverage requires them back; see follow-up).
