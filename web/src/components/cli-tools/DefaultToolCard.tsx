...
export default function DefaultToolCard({ 
  ...
  const handleSelectModel = (model: { value: string }): void => {
    setModelValue(model.value);
  };

  const hasActiveProviders = activeProviders.length > 0;

  const renderApiKeySelector = (): React.ReactNode => {
    return (
  ...
  const renderModelSelector = (): React.ReactNode => {
    return (
      <div className="mt-2 flex flex-col sm:flex-row sm:items-center gap-2">
        <input
          type="text"
          value={modelValue}
          onChange={(e: ChangeEvent<HTMLInputElement>) => setModelValue(e.target.value)}
  ...
  const renderGuideSteps = (): React.ReactNode => {
