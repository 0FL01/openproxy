...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus }: CodexToolCardProps): React.ReactNode {
  ...
  const getDisplayUrl = (): string => customBaseUrl || `${baseUrl}/v1`;
  const hasCustomSelectedApiKey = selectedApiKey && !apiKeys.some((key) => key.key === selectedApiKey);

  const checkCodexStatus = async (): Promise<void> => {
    ...
    try {
      ...
      setCodexStatus({ installed: false, error: (error as Error).message });
    } finally {
  ...
  return (
    <Card padding="xs" className="overflow-hidden">
      <div className="flex items-start justify-between gap-3 hover:cursor-pointer sm:items-center" onClick={onToggle}>
        <div className="flex min-w-0 items-center gap-3">
          <div className="size-8 flex items-center justify-center shrink-0">
            <img src="/providers/codex.png" alt={tool.name} width={32} height={32} className="size-8 object-contain rounded-lg" sizes="32px" onError={(e: React.SyntheticEvent<HTMLImageElement>) => { (e.target as HTMLImageElement).style.display = "none"; }} />
          </div>
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <div className="relative w-full min-w-0">
                    <input
                      type="text"
                      value={subagentModel}
                      onChange={(e: ChangeEvent<HTMLInputElement>) => setSubagentModel(e.target.value)}
      ...
      <ModelSelectModal
        ...
        onSelect={(model: { value: string }) => { setSubagentModel(model.value); setSubagentModalOpen(false); }}
