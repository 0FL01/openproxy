...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, initialStatus }: OpenCodeToolCardProps): React.ReactNode {
  ...
  useEffect(() => {
    ...
    if (status?.config?.agent?.explorer?.model?.startsWith("openproxy/")) {
      setSubagentModel(status.config.agent.explorer.model.replace("openproxy/", ""));
    }
  }, [status]);
      ...
      content: JSON.stringify({
        ...
        agent: {
          explorer: {
            ...
            mode: "subagent",
            model: `openproxy/${effectiveSubagentModel}`
          }
        }
      }, null, 2),
    }];
      ...
      <div className="flex items-start justify-between gap-3 hover:cursor-pointer sm:items-center" onClick={onToggle}>
        <div className="flex min-w-0 items-center gap-3">
          <div className="size-8 flex items-center justify-center shrink-0">
            <img src="/providers/opencode.png" alt={tool.name} width={32} height={32} className="size-8 object-contain rounded-lg" sizes="32px" onError={(e: React.SyntheticEvent<HTMLImageElement>) => { (e.target as HTMLImageElement).style.display = "none"; }} />
          </div>
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current base URL */}
                {status?.config?.provider?.["openproxy"]?.options?.baseURL && (
