...
export default function GrokBuildToolCard({
  ...
  useEffect(() => {
    ...
    if (isExpanded) void fetchModelAliases();
  }, [isExpanded]);
  ...
  const checkStatus = async (): Promise<void> => {
    ...
    try {
      ...
      setGrokStatus({ installed: false, error: (error as Error).message });
    } finally {
      setChecking(false);
    }
  };
  ...
  const getDisplayUrl = (): string => getEffectiveBaseUrl();
...
`;
    ...
    return [{ filename: "~/.grok/config.toml", content: tomlContent }];
  };
            ...
            <>
              <div className="flex flex-col gap-2">
                ...
                {grokStatus?.settings?.model?.base_url && (
                  <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                    ...
                    <span className="min-w-0 truncate rounded bg-surface/40 px-2 py-2 text-xs text-text-muted sm:py-1.5">
                      {grokStatus.settings.model.base_url}
                      {grokStatus.settings.model.model
                        ? ` · ${grokStatus.settings.model.model}`
                        : ""}
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-start sm:gap-2">
                  <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm pt-1.5">
                    Select Endpoint
                  </span>
                ...
                <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                  ...
                  <code className="text-xs font-mono text-text-muted">grok</code>
                  <code className="text-xs font-mono text-text-muted">
                    /model {MODEL_SLOT}
                  </code>
                  <code className="text-xs font-mono text-text-muted">
                    grok --model {selectedModel || "provider/model-id"}
                  </code>
                </div>
              </div>
