...
interface AddApiKeyModalProps {
  ...
  providerName?: string;
  proxyPools?: ProxyPool[];
  onSave: (data: { name: string; apiKey: string; priority: number; proxyPoolId?: string | null; testStatus: string }) => Promise<void>;
  onClose: () => void;
}

function AddApiKeyModal({ isOpen, provider, providerName, proxyPools, onSave, onClose }: AddApiKeyModalProps) {
  const NONE = "__none__";
  const [formData, setFormData] = useState<{ name: string; apiKey: string; priority: number; proxyPoolId: string }>({ name: "", apiKey: "", priority: 1, proxyPoolId: NONE });
    ...
    <Modal isOpen={isOpen} title={`Add ${providerName || provider} API Key`} onClose={onClose}>
      <div className="flex flex-col gap-4">
        <div>
          ...
          <input className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background focus:outline-none focus:border-primary" value={formData.name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({ ...formData, name: e.target.value })} placeholder="Production Key" />
        </div>
        <div className="flex gap-2">
          <div className="flex-1">
            <label className="text-xs text-text-muted mb-1 block">API Key</label>
            <input type="password" className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background focus:outline-none focus:border-primary" value={formData.apiKey} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({ ...formData, apiKey: e.target.value })} />
          </div>
          <div className="pt-6">
        ...
        <div>
          ...
          <input type="number" className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background focus:outline-none focus:border-primary" value={formData.priority} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({ ...formData, priority: Number.parseInt(e.target.value) || 1 })} />
        </div>
...
export default function ConnectionsCard({ providerId, isOAuth = false }: ConnectionsCardProps) {
  ...
  const handleSaveApiKey = async (formData: { name: string; apiKey: string; priority: number; proxyPoolId?: string | null; testStatus: string }) => {
    try {
  ...
  const handleUpdateConnection = async (formData: Record<string, any>) => {
    try {
