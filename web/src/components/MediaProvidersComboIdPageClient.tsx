...
const EXAMPLE_BODIES: Record<string, (n: string) => Record<string, any>> = {
  webSearch: (n: string) => ({ model: n, query: "What is the latest news about AI?", search_type: "web", max_results: 5 }),
  webFetch: (n: string) => ({ model: n, url: "https://example.com", format: "markdown" }),
  image: (n: string) => ({ model: n, prompt: "A cute cat playing piano", n: 1, size: "1024x1024" }),
  tts: (n: string) => ({ model: n, input: "Hello, this is a test.", voice: "alloy" }),
};

function getListingHref(kind: string) {
...
export default function MediaProvidersComboIdPageClient() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      if (keysRes.ok) {
        ...
        setApiKey((k.keys || []).find((x: any) => x.isActive !== false)?.key || "");
      }
  ...
  const saveCombo = async (patch: Record<string, any>) => {
  ...
  const handleDelete = async () => {
    ...
    if (res.ok) window.location.href = getListingHref(combo.kind);
  };
  ...
  const kindLabel = KIND_LABELS[combo.kind] || MEDIA_PROVIDER_KINDS.find((k: any) => k.id === combo.kind)?.label || "Combo";
  ...
  const curlExample = examplePath
    ? `curl -X POST http://localhost:4623${examplePath} \\\n  -H "Content-Type: application/json" \\\n  -H "Authorization: Bearer ${apiKey || "YOUR_KEY"}" \\\n  -d '${JSON.stringify(exampleBody)}'`
    : "";
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3 min-w-0">
          <a href={backHref} className="text-text-muted hover:text-brand-coral">
            <span className="material-symbols-outlined">arrow_back</span>
          </a>
          <div className="size-10 rounded-mini-lg bg-brand-coral/10 flex items-center justify-center">
            <span className="material-symbols-outlined text-brand-coral">layers</span>
          </div>
      ...
      <Card>
        ...
        {providers.length === 0 ? (
          <div className="text-center py-6 border border-dashed border-hairline rounded-mini-lg text-text-muted text-sm">
            No providers yet.
          </div>
        ) : (
          <div className="flex flex-col gap-2">
            {providers.map((entry: string, idx: number) => {
              ...
              return (
                <div key={`${entry}-${idx}`} className="flex items-center gap-3 p-2 rounded-mini-lg bg-black/[0.02] dark:bg-white/[0.02]">
                  ...
                  <div className="flex items-center gap-0.5">
                    <button onClick={() => handleMove(idx, -1)} disabled={idx === 0} className={`p-1 rounded ${idx === 0 ? "text-text-muted/20" : "text-text-muted hover:text-brand-coral hover:bg-black/5"}`} title="Move up">
                      <span className="material-symbols-outlined text-[16px]">arrow_upward</span>
                    </button>
                    <button onClick={() => handleMove(idx, 1)} disabled={idx === providers.length - 1} className={`p-1 rounded ${idx === providers.length - 1 ? "text-text-muted/20" : "text-text-muted hover:text-brand-coral hover:bg-black/5"}`} title="Move down">
      ...
      {combo.kind && examplePath && (
        <Card>
          ...
          <pre className="text-xs font-mono bg-black/[0.03] dark:bg-white/[0.03] p-3 rounded-mini-lg overflow-x-auto whitespace-pre-wrap break-all">
            {curlExample}
          </pre>
          {testError && (
          ...
          {testResult && (
            <div className="mt-3 flex flex-col gap-3">
              ...
              {testResult.imageUrl && (
                <div>
                  <div className="flex items-center justify-end mb-1.5">
                    <a href={testResult.imageUrl} download="image.png" className="inline-flex items-center gap-1 text-xs text-text-muted hover:text-brand-coral transition-colors">
                      <span className="material-symbols-outlined text-[14px]">download</span>
                      Download
                    </a>
                  </div>
                  <img src={testResult.imageUrl} alt="Generated" className="max-w-full rounded-mini-lg border border-hairline" />
                </div>
              )}
              {testResult.audioUrl && (
                <div>
                  <div className="flex items-center justify-end mb-1.5">
                    <a href={testResult.audioUrl} download="speech.mp3" className="inline-flex items-center gap-1 text-xs text-text-muted hover:text-brand-coral transition-colors">
                      <span className="material-symbols-outlined text-[14px]">download</span>
                      Download
                    </a>
                  </div>
              ...
              {testResult.json && (
                <pre className="text-xs font-mono bg-black/[0.03] dark:bg-white/[0.03] p-3 rounded-mini-lg overflow-auto max-h-[300px] whitespace-pre-wrap break-all">
                  {testResult.json}
                </pre>
              )}
            </div>
