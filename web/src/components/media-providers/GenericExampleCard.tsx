...
import { Row, KIND_EXAMPLE_CONFIG, getModelKind } from "./exampleShared";
...
function getImageEditDefaults(providerId: string, modelId: string) {
...
function toImagePreviewSrc(value: string) {
...
export function GenericExampleCard({ providerId, kind }: { providerId: string; kind: string }) {
  ...
  const safeExConfig = exConfig || {} as any;

  const kindModels = getModelsByProviderId(providerId).filter((m: any) => getModelKind(m) === kind);
  ...
  const selectedModelObj = kindModels.find((m: any) => m.id === selectedModel);
  const supportsEdit = !!(selectedModelObj as any)?.capabilities?.includes("edit");
  const supportsMask = !!(selectedModelObj as any)?.capabilities?.includes("mask");
  ...
  const [connections, setConnections] = useState<Array<{ id: string; name?: string; email?: string; provider: string; isActive?: boolean; providerSpecificData?: { chatgptPlanType?: string } }>>([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/providers/client")
      .then((r) => r.json())
      .then((d: any) => {
        const conns = (d.connections || []).filter((c: any) => c.provider === providerId && c.isActive !== false);
  ...
  const extraBodyFromFields = Object.entries(extraValues).reduce<Record<string, any>>((acc, [k, v]) => {
      ...
      if (isSse && res.body) {
        ...
        while (true) {
          ...
          while ((sep = buf.indexOf("\n\n")) !== -1) {
            ...
            buf = buf.slice(sep + 2);
            let evt: string | null = null, dataStr = "";
    ...
    <Card>
      ...
      <div className="flex flex-col gap-2.5">
            ...
            <div className="flex flex-col gap-2">
              ...
              {refImagePreviewSrc && (
                <img
                  ...
                  alt="Reference"
                  className="max-h-40 rounded-mini-lg border border-hairline object-contain bg-surface-card"
        ...
        {(exConfig.extraFields || [])
          .filter((f: any) => kindModels.length === 0 || (Array.isArray((selectedModelObj as any)?.params) && (selectedModelObj as any).params.includes(f.key)))
          .map((f: any) => (
        ...
        {(running || progress) && useStreaming && (
          <div className="flex flex-col gap-2 px-3 py-2 rounded-mini-md bg-surface-card border border-hairline sm:flex-row sm:items-center sm:gap-3">
            <span className="material-symbols-outlined text-[16px] text-brand-coral" style={running ? { animation: "spin 1s linear infinite" } : undefined}>
        ...
        {partialImage?.b64_json && !result && (
          <div>
            ...
            <img
              ...
              alt="Partial"
              className="max-w-full rounded-mini-lg border border-hairline mt-1.5 opacity-80"
            />
          </div>
        )}
          ...
          {kind === "image" && (binaryImageUrl || result?.data?.data?.[0]) && (
            <div className="mt-2">
              ...
              <img
                ...
                alt="Generated"
                className="max-w-full rounded-mini-lg border border-hairline"
              />
            </div>
          )}
        </div>
