...
- Closes: R7 / C42.
- Smallest next action: Rerun the C02 baseline matrix on the final tree, record baseline-vs-final with equal workload/features/logging, and close the tracker in docs/lean-proxy-results.md.
- Expected evidence: five-plus latency repetitions, no history-driven state growth, cross-provider goldens, auth races, and logging guarantees green; conditionals done or evidence-backed.
- Stop or replan if: Any wire/security/data contract breaks; block release, revert the offending commit, re-baseline.
...
- Next: C42 final like-for-like acceptance, measurements, and tracker close.
